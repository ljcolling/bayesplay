...
visual_compare <- function(model1, model2, n = 101, type = NULL) {
    ...
    # TODO: Plot range should be the widest of the two prior ranges
    # plot_range <- model1@likelihood_obj@plot$range
    plot_range1 <- m1@prior_obj@plot$range
    plot_range2 <- m0@prior_obj@plot$range
    plot_range <- c(min(plot_range1[1], plot_range2[1]),
   ...
   observation <- likelihood_obj@observation
    ...
    difference_df <- data.frame(x = c(observation),
                                 y = c(model1_func(observation)/
                                       model2_func(observation)))
   ...
   if (is.null(type)) {
    ...
    ggplot2::ggplot() +
      ...
      ggplot2::geom_point(data = difference_df,
      ggplot2::aes(x = x, y = y)) +
      ggplot2::xlim(plot_range) +

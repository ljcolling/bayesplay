...
```{r}
norm_mod <- likelihood(distribution = "normal", mean = 5.5, sd = 32.35)
norm_mod
```
...
```{r}
t_mod <- likelihood(distribution = "student_t", mean = 10, sd = 5, df = 15)
t_mod
```
...
```{r}
point_prior <- prior(distribution = "point", point = 0)
plot(point_prior)
```
...
```{r}
uniform_prior <- prior(distribution = "uniform", min = 10, max = 20)
plot(uniform_prior) 
```
...
```{r}
normal_prior <- prior(distribution = "normal", mean = 10, sd = 10)
plot(normal_prior)
```
...
```{r}
half_normal_prior <- prior(distribution = "normal", mean = 10, sd = 10, range = c(10, Inf))
